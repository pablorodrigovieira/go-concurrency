...
func (au *AuctionUseCase) FindAuctions(
	...
	var auctionStatus *auction_entity.AuctionStatus
	if status != nil {
		convertedStatus := auction_entity.AuctionStatus(*status)
		auctionStatus = &convertedStatus
	}

	auctionEntities, err := au.auctionRepositoryInterface.FindAuctions(
		ctx, auctionStatus, category, productName)
	if err != nil {
		return nil, err
	}
